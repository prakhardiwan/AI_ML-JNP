...
// #include "tensor.h"
#include "binary_fn.c"
#include "createTensor/src/createTensor.c"
...
void main(){
	...
	fillTensorValues(a, num_elems, 0);
	fillTensorValues(b, num_elems, 1);

	//call the function
	binaryOperatorOnTensor(a, b, operation);
	...
	for( ; elements_left > 0; elements_left -= MAX_SIZE_OF_REQUEST_IN_WORDS){
		...
		//change expected 
		double expected_result = 2*i+ 1 + 2*iter*MAX_SIZE_OF_REQUEST_IN_WORDS;   //or load from file
		switch(dataType){
			case u8: ; 
